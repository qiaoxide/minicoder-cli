...
import { colors, symbols } from '../ui/format.js';
  ...
  async execute(args: CommandArgs): Promise<void> {
    ...
    while (true) {
      ...
      try {
        ...
        await printAiMessage(response, { modelName: model, showThinking: true });

        messages.push({ role: 'model', content: response });
      } catch (error) {
        printChatError(String(error));
...
export class AskCommand implements Command {
  ...
  async execute(args: CommandArgs): Promise<void> {
    ...
    try {
      ...
      await printAiMessage(response, { modelName: model, showThinking: false });
    } catch (error) {
